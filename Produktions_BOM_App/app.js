...
function processExcel(arrayBuffer) {
  ...
  for (let r = range.s.r + 1; r <= range.e.r; r++) {
    ...
    const bomId = String(cellD.v).trim();
    const menge = cellF ? (parseFloat(cellF.v) || parseFloat(cellF.w) || 0) : 0;

    if (!bomId || bomId === "Aktivität") continue;

    if (!bomId || bomId === "" || bomId === "0") continue;
