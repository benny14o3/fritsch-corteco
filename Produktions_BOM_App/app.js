...
function printAsTabelle(selected) {
  ...
  const rows = selected.map(a => {
    ...
    return `
      <tr>
        ...
        <td style="font-weight:700;text-align:center;">${a.menge}</td>
  ...
  printWindow.document.write(`
    <html><head><title>Produktionsübersicht</title>
    <style>
      ...
      td { padding:5px 8px; border-bottom:1px solid #ddd; font-size:9pt; vertical-align:top; }
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Beschreibung</th>
            <th>Menge</th>
            <th>Arbeitsanweisung</th>
            <th>Verpackung</th>
            <th>Neutralisierung</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
...
function processExcel(arrayBuffer) {
  const workbook  = XLSX.read(arrayBuffer, { type: "array", cellFormula: false, cellNF: false });

  // Sheet "Aktivitätenliste" bevorzugen
  const sheetName = workbook.SheetNames.find(n => n.toLowerCase().includes("aktivit")) || workbook.SheetNames[0];
  const ws        = workbook.Sheets[sheetName];

  // Zellen direkt lesen um Formel-Cached-Values zu bekommen
  // Spalte D = Aktivität, Spalte F = Produzierbare Menge
  const auftraege = [];
  const range     = XLSX.utils.decode_range(ws["!ref"] || "A1");

  for (let r = range.s.r + 1; r <= range.e.r; r++) {
    const cellD = ws[XLSX.utils.encode_cell({ r, c: 3 })]; // Spalte D
    const cellF = ws[XLSX.utils.encode_cell({ r, c: 5 })]; // Spalte F

    if (!cellD || !cellD.v) continue;

    // BOM-ID als String ohne .0 (Excel liefert manchmal 80005007.0)
    let bomId = String(cellD.v).trim();
    if (bomId.endsWith(".0")) bomId = bomId.slice(0, -2);
    // Falls Zahl mit Dezimalstelle: abrunden
    ...
    // BOM in Datenbank suchen – Vergleich als String
    const bom = data.find(b => String(b.bom_id) === bomId);
    if (auftraege.length === 0) console.log("DEBUG erste BOM:", bomId, "data länge:", data.length, "erster eintrag:", data[0]?.bom_id);
...
function renderUploadModal(auftraege) {
      ...
      <tbody>
        ${auftraege.map((a, i) => {
          ...
          <tr style="${!a.bom ? 'background:#fff7ed;' : ''}">
            ...
            <td style="font-weight:700;font-size:13px;">${a.bomId}</td>
            <td style="color:#6b7280;font-size:12px;">${a.bom ? a.bom.beschreibung : "–"}</td>
            <td>
              <input type="number" class="comp-input comp-menge upload-menge"
                data-idx="${i}" value="${a.menge}" min="1"
                style="width:70px;font-weight:700;">
            </td>
            <td style="font-size:12px;color:#9a3412;">
              ${verpack ? `📦 ${verpack}` : '<span style="color:#d1d5db;">–</span>'}
            </td>
            <td>
              ${a.bom
                ? '<span style="color:#15803d;font-weight:700;font-size:12px;">✓</span>'
                : '<span style="color:#b91c1c;font-weight:700;font-size:12px;">⚠</span>'}
            </td>
          </tr>`;
        }).join("")}
      </tbody>
  ...
  // Menge-Änderungen live in auftraege speichern
  body.querySelectorAll(".upload-menge").forEach(input => {
    input.addEventListener("change", e => {
      const idx = parseInt(e.target.dataset.idx);
      auftraege[idx].menge = parseFloat(e.target.value) || 0;
    });
  });
...
function printSelected(mode = "auftraege") {
