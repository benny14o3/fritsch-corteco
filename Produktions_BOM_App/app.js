...
async function saveToStorage() {
  ...
  try {
    // Token zur Laufzeit entschlüsseln (XOR mit Admin-Passwort)
    const key = (ADMIN_PASSWORD.repeat(100)).slice(0, GITHUB_TOKEN_ENC.length);
    const decoded = atob(GITHUB_TOKEN_ENC);
    const token = Array.from(decoded).map((c, i) =>
      String.fromCharCode(c.charCodeAt(0) ^ ADMIN_PASSWORD.charCodeAt(i % ADMIN_PASSWORD.length))
    ).join("");

    // SHA holen
    const infoRes = await fetch(
      `https://api.github.com/repos/${GITHUB_REPO}/contents/${GITHUB_FILE}?ref=${GITHUB_BRANCH}&_=${Date.now()}`,
      { headers: { Authorization: `token ${token}`, "User-Agent": "fritsch-bom-app" } }
    );
