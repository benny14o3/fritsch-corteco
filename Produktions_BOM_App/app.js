...
function renderResults(boms) {
  ...
  boms.forEach((bom, index) => {
    ...
    card.innerHTML = `
      ...
      ${bom.bild_url ? `
        <div class="bom-bild-container">
          <img src="${bom.bild_url}" style="max-width:100%;max-height:300px;border-radius:8px;margin:12px 0;display:block;">
        </div>` : ""}
...
function saveEdit() {
  ...
  const bildUrl     = document.getElementById("modal-image-url").value || "";
  const updatedBom  = { bom_id: bomId, beschreibung, arbeitsanweisung: arbeits, neutralisierung: neutral, components, bild_url: bildUrl };
...
function printSelected(mode) {
  ...
  const pages = selected.map(a => {
    ...
    return `<div style="page-break-after:always;font-family:Arial,sans-serif;padding:15px;font-size:10pt;">
      ...
      ${bom.bild_url ? `<div style="margin:10px 0;"><img src="${bom.bild_url}" style="max-width:100%;max-height:250px;border-radius:6px;"></div>` : ""}
...
function renderImagePreview(url) {
  ...
  const removeBtn = document.getElementById("modal-image-remove");
  if (!preview) return;
  if (url) {
